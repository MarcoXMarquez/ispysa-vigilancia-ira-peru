...
OUTPUT_DIR = ROOT_DIR / "outputs" / "adults_feature_importance_ci"
...
def load_data():
    ...
    if TARGET_COL not in ira.columns:
        raise KeyError(f"{TARGET_COL} not found in IRA file.")
...
def save_feature_importance(region, model_name, model, feature_cols):
    ...
    plt.ylabel("Feature")
    plt.title(f"{region} Adults 60+ - {model_name} Feature Importance")
    plt.tight_layout()
...
def run_region(region_df, region):
    ...
    for model_name in ["random_forest", "xgboost"]:
        ...
        while start + FORECAST_HORIZON <= len(df_feat):
            ...
            # Save feature importance from the final rolling windows repeatedly.
            # The final saved file will reflect the latest trained window.
            save_feature_importance(region, model_name, model, feature_cols)

            if model_name == "random_forest":
                pred_model, lower_model, upper_model = random_forest_interval(model, X_test)
            else:
                pred_model, lower_model, upper_model = bootstrap_xgb_interval(X_train, y_train, X_test)

            pred_original = inverse_transform_y(pred_model)
            lower_original = inverse_transform_y(lower_model)
            upper_original = inverse_transform_y(upper_model)
            ...
            lower_original = np.clip(lower_original, 0, upper_clip)
            upper_original = np.clip(upper_original, 0, upper_clip)

            for i in range(len(test_df)):
                all_results.append({
                    "date": str(test_dates[i])[:10],
                    "actual": y_test_original[i],
                    "predicted": pred_original[i],
                    "lower_ci": lower_original[i],
                    "upper_ci": upper_original[i],
                    "model": model_name,
                    "region": region
                })
    ...
    csv_path = OUTPUT_DIR / f"{region.lower()}_adults_ml_predictions_with_ci.csv"
...
def plot_ci(region, results_df):
    ...
    plt.title(f"{region} Adults 60+ - RF/XGBoost Predictions with Confidence Bands")
    plt.xlabel("Date")
    ...
    plot_path = OUTPUT_DIR / f"{region.lower()}_adults_ml_confidence_bands.png"
    plt.savefig(plot_path, dpi=300, bbox_inches="tight")
    plt.close()
