...
OUTPUT_DIR = ROOT_DIR / "outputs" / "adults_cases_lstm_top3"
...
def load_data():
    ...
    if TARGET_COL not in ira.columns:
        raise KeyError(
            f"{TARGET_COL} not found. Available columns are:\n{ira.columns.tolist()}"
        )
...
class LSTMRegionTrainer:
    ...
    def forecast_future(self):
        ...
        for _ in range(FUTURE_BLOCKS):
            ...
            block_preds = np.clip(block_preds, 0, None)
    ...
    def plot_results(self, future_df, output_png):
       ...
       plt.title(
           f"{self.region_name} | Adults 60+ Cases | LSTM | "
...
def main():
    ...
    for region in REGIONS:
        ...
        results_df.to_csv(
            OUTPUT_DIR / f"{region_prefix}_adults_cases_lstm_predictions.csv",
            index=False
        )

        metrics_df.to_csv(
            OUTPUT_DIR / f"{region_prefix}_adults_cases_lstm_metrics.csv",
            index=False
        )

        future_df.to_csv(
            OUTPUT_DIR / f"{region_prefix}_adults_cases_lstm_future_predictions.csv",
            index=False
        )

        trainer.plot_results(
            future_df=future_df,
            output_png=OUTPUT_DIR / f"{region_prefix}_adults_cases_lstm_plot.png"
        )
    ...
    final_results.to_csv(
        OUTPUT_DIR / "all_regions_adults_cases_lstm_predictions.csv",
        index=False
    )

    final_metrics.to_csv(
        OUTPUT_DIR / "all_regions_adults_cases_lstm_metrics.csv",
        index=False
    )

    final_future.to_csv(
        OUTPUT_DIR / "all_regions_adults_cases_lstm_future_predictions.csv",
        index=False
    )
