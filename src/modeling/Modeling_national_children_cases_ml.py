...
OUTPUT_DIR = ROOT_DIR / "outputs" / "national_children_cases_ml"
...
def inverse_transform_y(y):
    y = np.asarray(y, dtype=float)
    return np.clip(np.expm1(y), 0, None) if USE_LOG_TARGET else np.clip(y, 0, None)
...
def prepare_national_series(ira, pop):
    ...
    weekly["incidence_raw"] = weekly["incidence_raw"].interpolate(limit_direction="both")
...
def make_next_feature_row(history, next_date):
    ...
    row = {
        ...
        "roll_std_4": np.std(y_model[-4:], ddof=1),

        "roll_mean_8": np.mean(y_model[-8:]),
        "roll_std_8": np.std(y_model[-8:], ddof=1),
...
class NationalMLTrainer:
    ...
    def plot_results(self, future_df, output_png):
        ...
        plt.title(
            "National Children <5 Cases | ML Models | "
...
def main():
    ...
    national_df.to_csv(
        OUTPUT_DIR / "national_children_cases_weekly_series.csv",
        index=False
    )
    ...
    results_df.to_csv(
        OUTPUT_DIR / "national_children_cases_ml_predictions.csv",
        index=False
    )

    metrics_df.to_csv(
        OUTPUT_DIR / "national_children_cases_ml_metrics.csv",
        index=False
    )

    future_df.to_csv(
        OUTPUT_DIR / "national_children_cases_ml_future_predictions.csv",
        index=False
    )

    trainer.plot_results(
        future_df=future_df,
        output_png=OUTPUT_DIR / "national_children_cases_ml_plot.png"
    )
